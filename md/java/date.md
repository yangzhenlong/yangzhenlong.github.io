...
public class BizTimeMapper {
    private static long TIME_MS_SECOND = 1000L;
    private static long TIME_MS_MINUTE;
    private static long TIME_MS_HOUR;
    private static long TIME_MS_DAY;
    private static long TIME_MS_MONTH;
    private static long TIME_MS_YEAR;
    private static String format;

    private BizTimeMapper() {
    }

    /**获取业务时间*/
    private static String getBizTimeString(long start, long end) {
        long time = end - start;
        long months;
        if (time < TIME_MS_HOUR) {
            months = time / TIME_MS_MINUTE;
            return Math.max(1L, months) + "分钟前";
        } else if (time < TIME_MS_DAY) {
            months = time / TIME_MS_HOUR;
            return months + "小时前";
        } else if (time < TIME_MS_MONTH) {
            months = time / TIME_MS_DAY;
            return months == 1L ? "昨天" : months + "天前";
        } else if (time < TIME_MS_YEAR) {
            months = time / TIME_MS_MONTH;
            return months + "个月前";
        } else {
            return DateFormatUtils.format(new Date(start), format);
        }
    }

    static {
        TIME_MS_MINUTE = 60L * TIME_MS_SECOND;
        TIME_MS_HOUR = 60L * TIME_MS_MINUTE;
        TIME_MS_DAY = 24L * TIME_MS_HOUR;
        TIME_MS_MONTH = 30L * TIME_MS_DAY;
        TIME_MS_YEAR = 12L * TIME_MS_MONTH;
